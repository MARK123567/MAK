...
//assembly settings
assemblyJarName in assembly := s"mak-all-${version.value}.jar"
...
dockerfile in docker := {
  ...
  val startmak = (sourceDirectory in IntegrationTest).value / "container" / "start-mak.sh"

  new Dockerfile {
    from("anapsix/alpine-java:8_server-jre")
    add(assembly.value, "/opt/mak/mak.jar")
    add(Seq(configTemplate, startmak), "/opt/mak/")
    run("chmod", "+x", "/opt/mak/start-mak.sh")
    entryPoint("/opt/mak/start-mak.sh")
  }
}
...
inConfig(Linux)(Seq(
  ...
  packageDescription := "mak full node"
))
...
mappings in Universal += (baseDirectory.value / s"mak-${network.value}.conf" -> "doc/mak.conf.sample")
